...
size_t FileHandler::WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {
    ...
    buffer->insert(buffer->end(), static_cast<uchar*>(contents), 
                   static_cast<uchar*>(contents) + total_size);
    return total_size;
}

bool FileHandler::DownloadImageFromURL(const std::string& url, cv::Mat& image, int timeout_seconds) {
    ...
    curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);  // 在生产环境中应该设为1L
    ...
    LOG_INFO("Successfully downloaded image from URL: {}x{}", image.cols, image.rows);
    return true;
}
