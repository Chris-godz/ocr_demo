...
public:
     ...
     * @return 成功返回true
     */
    static bool DownloadImageFromURL(const std::string& url, cv::Mat& image, int timeout_seconds = 10);
     ...
     * @return 生成的文件名（不含路径）
     */
    static std::string SaveVisualizationImage(const cv::Mat& image, const std::string& output_dir = "/tmp/ocr_vis");
