...
def create_ocr_models(use_doc_preprocessing=True):
    """
    ...
    """
    dir_name = "engine/model_files/best"
    det_model_path = dir_name
    # det_model_path = "engine/model_files/v5/det_v5_640.dxnn"
    cls_model_path = f"{dir_name}/textline_ori.dxnn"  # 기존 DXNN 모델 (주석 처리)
    rec_model_dirname = dir_name
    doc_ori_model_path = f"{dir_name}/doc_ori.dxnn"
    ...
    return det_model, cls_model, rec_models, doc_ori_model, doc_unwarping_model


def create_ocr_workers(num_workers=3, use_doc_preprocessing=True, use_doc_orientation=True):
    """
    ...
    print("Creating OCR models...", num_workers, "use_unwarping:", use_doc_preprocessing, "use_doc_ori:", use_doc_orientation)
    det_model, cls_model, rec_models, doc_ori_model, doc_unwarping_model = create_ocr_models(
...
def create_async_ocr_pipeline(use_doc_preprocessing=True, use_doc_orientation=True):
    ...
    det_models, cls_model, rec_models, doc_ori_model, doc_unwarping_model = create_ocr_models(
        use_doc_preprocessing=use_doc_preprocessing
    )
